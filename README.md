# Hey, sono Flosh 👋

**Java/Python • CLI & backend • Raspberry Pi tinkerer • Applied AI/automation**

- 🎓 Studente di Informatica
- 🧰 Mi piace costruire **tool da terminale** e piccoli backend
- 🥧 Uso il **Raspberry Pi** come palestra per automazione e servizi self-hosted
- 📚 In questo periodo: Python “reboot”, Java, SQL/PostgreSQL, Sistemi Operativi

---

## 🚀 Progetti in evidenza
- **Menu Queen** — Menu Ristorante&Pizzeria QUEEN 
  👉 <https://github.com/floshdev/menu-queen>

- **pikeyvault** — CLI password vault (Python, scrypt + Fernet) *(in arrivo)*  
  👉 <https://github.com/floshdev/pikeyvault>

- **TheKnife (mirror, read-only)** — progetto universitario (solo vetrina) 
  👉 link al fork/mirror quando pronto

---

## 🧪 Stack & strumenti
`Java, C, C++` • `Python` • `SQL (PostgreSQL/SQLite)` • `Bash`  
`Git/GitHub` • `VS Code` • `Linux` • `Raspberry Pi`

---

## 📬 Contatti
Per ora scrivimi qui su GitHub. 
